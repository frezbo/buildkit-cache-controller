...
#
# Generated on 2026-05-14T18:32:00Z by kres 1762ab2.
...
all: unit-tests buildkit-cache-controller image-buildkit-cache-controller cb-hook image-cb-hook helm lint
...
generate: helm-plugin-install  ## Generate .proto definitions.
	...
	if echo "$$TAG" | grep -qE '^v[0-9]+\.[0-9]+\.[0-9]+$$'; then \
	  sed -i "s/^appVersion: .*/appVersion: \"$$TAG\"/" deploy/helm/buildkit-cache-controller/Chart.yaml; \
	  MINOR_PATCH=$$(echo "$$TAG" | sed 's/^v[0-9]*\.//'); \
	  sed -i "s/^version: .*/version: 0.$$MINOR_PATCH/" deploy/helm/buildkit-cache-controller/Chart.yaml; \
	fi
	@$(MAKE) helm-docs
...
.PHONY: $(ARTIFACTS)/buildkit-cache-controller-linux-amd64
$(ARTIFACTS)/buildkit-cache-controller-linux-amd64:
	@$(MAKE) local-buildkit-cache-controller-linux-amd64 DEST=$(ARTIFACTS)

.PHONY: buildkit-cache-controller-linux-amd64
buildkit-cache-controller-linux-amd64: $(ARTIFACTS)/buildkit-cache-controller-linux-amd64  ## Builds executable for buildkit-cache-controller-linux-amd64.

.PHONY: $(ARTIFACTS)/buildkit-cache-controller-linux-arm64
$(ARTIFACTS)/buildkit-cache-controller-linux-arm64:
	@$(MAKE) local-buildkit-cache-controller-linux-arm64 DEST=$(ARTIFACTS)

.PHONY: buildkit-cache-controller-linux-arm64
buildkit-cache-controller-linux-arm64: $(ARTIFACTS)/buildkit-cache-controller-linux-arm64  ## Builds executable for buildkit-cache-controller-linux-arm64.

.PHONY: buildkit-cache-controller
buildkit-cache-controller: buildkit-cache-controller-linux-amd64 buildkit-cache-controller-linux-arm64  ## Builds executables for buildkit-cache-controller.
...
.PHONY: image-buildkit-cache-controller
image-buildkit-cache-controller:  ## Builds image for buildkit-cache-controller.
	@$(MAKE) registry-$@ IMAGE_NAME="buildkit-cache-controller"

.PHONY: $(ARTIFACTS)/cb-hook-linux-amd64
$(ARTIFACTS)/cb-hook-linux-amd64:
	@$(MAKE) local-cb-hook-linux-amd64 DEST=$(ARTIFACTS)

.PHONY: cb-hook-linux-amd64
cb-hook-linux-amd64: $(ARTIFACTS)/cb-hook-linux-amd64  ## Builds executable for cb-hook-linux-amd64.

.PHONY: $(ARTIFACTS)/cb-hook-linux-arm64
$(ARTIFACTS)/cb-hook-linux-arm64:
	@$(MAKE) local-cb-hook-linux-arm64 DEST=$(ARTIFACTS)

.PHONY: cb-hook-linux-arm64
cb-hook-linux-arm64: $(ARTIFACTS)/cb-hook-linux-arm64  ## Builds executable for cb-hook-linux-arm64.

.PHONY: cb-hook
cb-hook: cb-hook-linux-amd64 cb-hook-linux-arm64  ## Builds executables for cb-hook.

.PHONY: image-cb-hook
image-cb-hook:  ## Builds image for cb-hook.
	@$(MAKE) registry-$@ IMAGE_NAME="cb-hook"

.PHONY: helm
helm: $(ARTIFACTS)  ## Package helm chart
	@helm package deploy/helm/buildkit-cache-controller -d $(ARTIFACTS)

.PHONY: helm-release
helm-release: helm  ## Release helm chart
	@helm push $(ARTIFACTS)/buildkit-cache-controller-*.tgz oci://$(HELMREPO) 2>&1 | tee $(ARTIFACTS)/.digest
	@cosign sign --yes $(COSIGN_ARGS) $(HELMREPO)/buildkit-cache-controller@$$(cat $(ARTIFACTS)/.digest | awk -F "[, ]+" '/Digest/{print $$NF}')

.PHONY: chart-lint
chart-lint:  ## Lint helm chart
	@helm lint deploy/helm/buildkit-cache-controller
...
chart-unittest: $(ARTIFACTS)  ## Run helm chart unit tests
	@helm unittest deploy/helm/buildkit-cache-controller --output-type junit --output-file $(ARTIFACTS)/helm-unittest-report.xml

.PHONY: chart-gen-schema
chart-gen-schema:  ## Generate helm chart schema
	@helm schema --use-helm-docs --draft=7 --indent=2 --values=deploy/helm/buildkit-cache-controller/values.yaml --output=deploy/helm/buildkit-cache-controller/values.schema.json
